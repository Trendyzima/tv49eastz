...
func chooseVideoProfile(caps []VideoCapability, preset CapturePreset, hevcAllowed bool) VideoProfile {
	...
	for _, c := range candidates {
		if !c.Stable || c.Width <= 0 || c.Height <= 0 { continue }
		fps := c.MaxFPS
		if preset != CapturePresetMaximum && fps > 30 { fps = 30 }
		if preset == CapturePresetLowLight && fps > 30 { fps = 30 }
		if fps < 24 { continue }
		codec := "avc"
		if hevcAllowed && c.HEVC { codec = "hevc" }
		bitrate := c.MaxBitrate
		if bitrate <= 0 { bitrate = defaultVideoBitrate(c.Width, c.Height, fps) }
...
func defaultVideoBitrate(width, height, fps int) int {
	pixels := width * height
	base := 8_000_000
	if pixels >= 3840*2160 { base = 50_000_000 } else if pixels >= 1920*1080 { base = 16_000_000 }
	if fps >= 60 { base = base * 3 / 2 }
	return base
}
...
func chooseAudioProfile(caps AudioCapability, preset CapturePreset, source string) AudioProfile {
	rate := 48000
	found := false
	for _, r := range caps.SampleRates { if r == 48000 { found = true; break } }
	if !found && len(caps.SampleRates) > 0 { rate = caps.SampleRates[0] }
	channels := caps.Channels
	if channels < 1 { channels = 1 }
	if channels > 2 { channels = 2 }
	if preset == CapturePresetVoicePriority || preset == CapturePresetCreator { channels = 1 }
	if source == "" { source = "auto" }
	return AudioProfile{SampleRate:rate, Channels:channels, Bitrate:192000, Source:source, NoiseSuppression:true, EchoCancellation:true}
}
...
func DecideCaptureQuality(caps []VideoCapability, audio AudioCapability, preset CapturePreset, hevcAllowed bool, source string) CaptureQualityDecision {
	...
	warnings := []string{}
	if video.Width >= 3840 && video.FPS >= 60 { warnings = append(warnings, "high thermal and storage load") }
	if audioProfile.SampleRate != 48000 { warnings = append(warnings, "48 kHz audio unavailable; using device-supported rate") }
