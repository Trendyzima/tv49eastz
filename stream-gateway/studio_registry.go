...
func (r *StudioRegistry) UpdateScene(sessionID string, scene StudioScene) (StudioRuntime, error) {
	...
	for i := range rt.Spec.Scenes {
		if rt.Spec.Scenes[i].ID == scene.ID {
			rt.Spec.Scenes[i] = cloneScene(scene)
			rt.UpdatedAt = time.Now().UTC()
			return cloneRuntime(*rt), nil
		}
	}
	rt.Spec.Scenes = append(rt.Spec.Scenes, cloneScene(scene))
	rt.UpdatedAt = time.Now().UTC()
	return cloneRuntime(*rt), nil
}
...
func (r *StudioRegistry) DeleteScene(sessionID, sceneID string) (StudioRuntime, error) {
	...
	if rt.Spec.ActiveScene == sceneID || rt.ProgramScene == sceneID {
		return StudioRuntime{}, errors.New("cannot delete program scene")
	}
	for i := range rt.Spec.Scenes {
...
func (r *StudioRegistry) SetPreviewScene(sessionID, sceneID string) (StudioRuntime, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	rt, ok := r.sessions[sessionID]
	if !ok {
	...
	if _, ok := findScene(rt.Spec.Scenes, sceneID); !ok {
		return StudioRuntime{}, errors.New("scene not found")
	}
	rt.PreviewScene = sceneID
	rt.UpdatedAt = time.Now().UTC()
	return cloneRuntime(*rt), nil
}

func (r *StudioRegistry) ActivateScene(sessionID, sceneID string) (StudioRuntime, error) {
	return r.TransitionScene(sessionID, sceneID, "cut", 0)
}

func (r *StudioRegistry) TransitionScene(sessionID, sceneID, style string, durationMS int) (StudioRuntime, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	rt, ok := r.sessions[sessionID]
	if !ok {
	...
	if durationMS < 0 || durationMS > 60000 {
		return StudioRuntime{}, errors.New("transition duration must be between 0 and 60000 ms")
	}
	from := rt.ProgramScene
	rt.PreviewScene = sceneID
	rt.Transition = StudioTransitionState{
		FromScene: from, ToScene: sceneID, Style: style, DurationMS: durationMS,
		StartedAt: time.Now().UTC(), Active: durationMS > 0,
	}
...
func (r *StudioRegistry) UpdateSource(sessionID string, source StudioSource) (StudioRuntime, error) {
	if err := source.Validate(); err != nil {
		return StudioRuntime{}, err
	}
	r.mu.Lock()
	...
	if _, ok := rt.Sources[source.ID]; !ok {
		return StudioRuntime{}, errors.New("source not found")
	}
	rt.Sources[source.ID] = source
	for i := range rt.Spec.Scenes {
		for j := range rt.Spec.Scenes[i].Sources {
			if rt.Spec.Scenes[i].Sources[j].ID == source.ID {
				rt.Spec.Scenes[i].Sources[j] = source
			}
		}
	}
	rt.UpdatedAt = time.Now().UTC()
...
func (r *StudioRegistry) RemoveSource(sessionID, sourceID string) (StudioRuntime, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	rt, ok := r.sessions[sessionID]
	if !ok {
	...
	for _, sc := range rt.Spec.Scenes {
		for _, src := range sc.Sources {
			if src.ID == sourceID {
				return StudioRuntime{}, errors.New("source is still referenced by a scene")
			}
		}
	}
	delete(rt.Sources, sourceID)
	rt.UpdatedAt = time.Now().UTC()
	return cloneRuntime(*rt), nil
}
...
func (r *StudioRegistry) SetAudio(sessionID string, audio []StudioAudioBus) (StudioRuntime, error) {
	...
	active.Audio = append([]StudioAudioBus(nil), audio...)
	if err := active.Validate(); err != nil {
		return StudioRuntime{}, err
	}
	for i := range rt.Spec.Scenes {
		if rt.Spec.Scenes[i].ID == active.ID {
			rt.Spec.Scenes[i] = cloneScene(active)
		}
	}
	rt.UpdatedAt = time.Now().UTC()
	return cloneRuntime(*rt), nil
...
func runtimeFromSpec(spec StudioSessionSpec) StudioRuntime {
	sources := map[string]StudioSource{}
	...
	return StudioRuntime{
		Spec: spec, Sources: sources, PreviewScene: spec.ActiveScene,
		ProgramScene: spec.ActiveScene, UpdatedAt: time.Now().UTC(),
	}
}

// cloneRuntime prevents callers from mutating registry state through slices or
// maps returned by Get/Create/update methods. This is essential for concurrent
// producer/control-deck clients sharing one in-memory session.
func cloneRuntime(in StudioRuntime) StudioRuntime {
	out := in
	out.Spec.Scenes = make([]StudioScene, len(in.Spec.Scenes))
	for i, scene := range in.Spec.Scenes {
		out.Spec.Scenes[i] = cloneScene(scene)
	}
...
func cloneScene(in StudioScene) StudioScene {
	out := in
	out.Sources = append([]StudioSource(nil), in.Sources...)
	out.Overlays = append([]StudioOverlay(nil), in.Overlays...)
	out.Audio = append([]StudioAudioBus(nil), in.Audio...)
	return out
}
