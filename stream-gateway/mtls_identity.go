...
func DeviceIdentityFromTLS(state *tls.ConnectionState, registry *DeviceRegistry) (DeviceRecord, error) {
	if state == nil || len(state.PeerCertificates) == 0 { return DeviceRecord{}, errors.New("missing verified device certificate") }
	if registry == nil { return DeviceRecord{}, errors.New("device registry unavailable") }
	cert := state.PeerCertificates[0]
	if cert == nil { return DeviceRecord{}, errors.New("missing device certificate") }
	d, ok := registry.Verify(cert)
	if !ok { return DeviceRecord{}, errors.New("device certificate not authorized") }
	return d, nil
}
...
func NewMutualTLSConfig(serverCert tls.Certificate, clientCAs *x509.CertPool) (*tls.Config, error) {
	if len(serverCert.Certificate) == 0 { return nil, errors.New("missing server certificate") }
	if clientCAs == nil { return nil, errors.New("missing client CA pool") }
