...
public final class PostManagerActivity extends AppCompatActivity {
    ...
    private void build() {
        ...
        root.setBackgroundColor(Color.rgb(248,245,251));

        TextView title = text("Your posts", 24, Color.rgb(38,29,48), true);
        root.addView(title, lp(48, 0));
        status = text("Loading…", 13, Color.DKGRAY, false);
        root.addView(status, lp(34, 0));
        ...
        create.setOnClickListener(v -> { startActivity(new android.content.Intent(this, ModernSocialActivity.class)); });
    ...
    private void refresh() {
        ...
        repo.loadFeed(100, r -> runOnUiThread(() -> {
            ...
            if (posts != null) for (SocialPost post : posts) {
    ...
    private void addPost(SocialPost post) {
        ...
        TextView body = text(post.getBody() == null || post.getBody().trim().isEmpty() ? "Media post" : post.getBody(), 16, Color.rgb(38,29,48), false);
        body.setGravity(Gravity.CENTER_VERTICAL);
        card.addView(body, lp(80, 0));
        TextView meta = text("Posted " + (post.getCreatedAt() == null ? "recently" : post.getCreatedAt()), 11, Color.DKGRAY, false);
        card.addView(meta, lp(30, 0));
        Button edit = new Button(this);
        edit.setText("Edit post");
        edit.setOnClickListener(v -> editPost(post));
        card.addView(edit, lp(50, 6));
    ...
    private void editPost(SocialPost post) {
        ...
        input.setTextColor(Color.rgb(38,29,48));
        ...
        new androidx.appcompat.app.AlertDialog.Builder(this)
                ...
                .create().setOnShowListener(dialog -> {
                    ...
                    d.getButton(androidx.appcompat.app.AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
                        ...
                        repo.editPost(post.getId(), body, r -> runOnUiThread(() -> {
                            if (r.getError() != null) {
