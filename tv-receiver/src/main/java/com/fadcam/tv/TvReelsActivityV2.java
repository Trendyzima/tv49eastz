...
public final class TvReelsActivityV2 extends AppCompatActivity {
        ...
        feedClient.load(new IptvFeedClientV2.Listener() {
            @Override public void onSuccess(List<IptvReel> result) {
                runOnUiThread(() -> {
                    ...
                    rebuildVisible();
                    adapter.notifyDataSetChanged();
                    if (currentPosition >= adapter.getItemCount()) currentPosition = 0;
            ...
            @Override public void onError(Exception error) {
                runOnUiThread(() -> {
                    ...
                    if (reels.isEmpty()) Toast.makeText(TvReelsActivityV2.this,
                    ...
                    feedClient.loadSource(url, "Added source", new IptvFeedClientV2.Listener() {
                        @Override public void onSuccess(List<IptvReel> result) {
                            runOnUiThread(() -> {
                                ...
                                rebuildVisible();
                                adapter.notifyDataSetChanged();
                                Toast.makeText(TvReelsActivityV2.this, result.size() + " channels added", Toast.LENGTH_SHORT).show();
                            });
                        }
                        @Override public void onError(Exception error) {
                            runOnUiThread(() -> Toast.makeText(TvReelsActivityV2.this, "Could not load source", Toast.LENGTH_LONG).show());
                        }
                    });
                }).setNegativeButton("Cancel", null).show();
    }
    ...
    private final class ReelPage extends FrameLayout {
        ...
        ReelPage(Context context) {
            ...
            share.setOnClickListener(v -> TvReelsActivityV2.this.share(item));
        ...
        private View scrim(boolean top) {
            View v = new View(TvReelsActivityV2.this);
