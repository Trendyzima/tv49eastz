...
/**
 * Native football data adapter. The endpoint is configurable so TV 49 East can move from a
 * public scoreboard feed to a licensed provider without changing the Android UI.
 */
public final class FootballRepository {
    ...
    public void loadMatches(String competition, String mode, CallbackResult callback) {
        ...
        if ("standings".equalsIgnoreCase(mode)) path = "standings";
        String base = BuildConfig.FOOTBALL_API_URL == null ? "" : BuildConfig.FOOTBALL_API_URL.trim();
        if (base.isEmpty()) {
            callback.onResult(new ArrayList<>(), new IOException("Football API is not configured"));
            return;
        }
    ...
    private List<FootballMatch> parseEvents(JsonObject root, String competition, String mode) {
        ...
        for (JsonElement e : events) {
            ...
            String state = text(event, "status", "type", "state", "name");
            String status = event.has("status") && event.get("status").isJsonObject() ? text(event.getAsJsonObject("status"), "type", "detail", "name") : state;
            if ("live".equalsIgnoreCase(mode) && !isLive(status)) continue;
            if ("results".equalsIgnoreCase(mode) && isLive(status)) continue;
            JsonArray competitions = event.has("competitions") ? event.getAsJsonArray("competitions") : new JsonArray();
            if (competitions.size() == 0) continue;
            JsonObject match = competitions.get(0).getAsJsonObject();
            JsonArray competitors = match.has("competitors") ? match.getAsJsonArray("competitors") : new JsonArray();
            ...
            for (JsonElement c : competitors) {
                ...
                if (isHome) { home = name; hs = score.isEmpty() ? "0" : score; }
                else { away = name; as = score.isEmpty() ? "0" : score; }
            }
            out.add(new FootballMatch(text(event, "id"), competition, home, away, hs, as,
                    text(event, "date"), status, text(match, "status")));
        }
        return out;
    }
    ...
    private List<FootballMatch> parseStandings(JsonObject root, String competition) {
        ArrayList<FootballMatch> out = new ArrayList<>();
        JsonArray children = root.has("children") ? root.getAsJsonArray("children") : new JsonArray();
        for (JsonElement child : children) {
            JsonObject group = child.getAsJsonObject();
            if (!group.has("standings")) continue;
            JsonArray entries = group.getAsJsonObject("standings").has("entries") ? group.getAsJsonObject("standings").getAsJsonArray("entries") : new JsonArray();
    ...
    private String standingsSummary(JsonObject row) {
        ...
        for (JsonElement stat : row.getAsJsonArray("stats")) {
            JsonObject o = stat.getAsJsonObject();
            ...
            if ("points".equalsIgnoreCase(name) || "wins".equalsIgnoreCase(name) || "ties".equalsIgnoreCase(name) || "losses".equalsIgnoreCase(name) || "pointDifferential".equalsIgnoreCase(name)) {
                ...
                s.append(name).append(" ").append(text(o, "displayValue", "value"));
            }
        }
        return s.toString();
    }
    ...
    private boolean isLive(String status) {
        ...
        return s.contains("live") || s.contains("in progress") || s.contains("halftime") || s.contains("second half");
    }
    ...
    private String text(JsonObject o, String... keys) {
