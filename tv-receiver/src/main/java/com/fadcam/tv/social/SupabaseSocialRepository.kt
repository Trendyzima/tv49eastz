...
class SupabaseSocialRepository(context: Context) {
    ...
    fun createPost(bodyText:String,mediaUrl:String?,mediaType:String?,callback:ResultCallback<SocialPost?>){val uid=currentUserId()?:return fail(callback,"Sign in required");val text=bodyText.trim();val media=mediaUrl?.trim().takeUnless{it.isNullOrEmpty()};if(text.isEmpty()&&media==null)return fail(callback,"Add some text or attach a photo/video");if(mediaType!=null&&mediaType !in setOf("image","video"))return fail(callback,"Unsupported post media type");val path="/rest/v1/posts?select=id,body,media_url,media_type,created_at,like_count,reply_count,repost_count,author:profiles!posts_author_id_fkey(id,username,display_name,avatar_url,bio)";val payload=mutableMapOf<String,Any>("author_id" to uid,"body" to text);if(media!=null)payload["media_url"]=media;if(mediaType!=null)payload["media_type"]=mediaType;mutate(path,"POST",json(payload),callback){parsePosts(it).firstOrNull()}}
    ...
    fun uploadMedia(uri:Uri,kind:String,callback:ResultCallback<String>){val token=currentAccessToken()?:return fail(callback,"Sign in required");val uid=currentUserId()?:return fail(callback,"Session user id missing");val size=try{app.contentResolver.openAssetFileDescriptor(uri,"r")?.use{it.length}?:-1L}catch(_:Throwable){-1L};if(size<0L)return fail(callback,"Unable to determine media size");if(size>20L*1024L*1024L)return fail(callback,"Media exceeds 20 MB");val mime=app.contentResolver.getType(uri)?:return fail(callback,"Unsupported media type");if(!mime.startsWith("image/")&&!mime.startsWith("video/"))return fail(callback,"Unsupported media type");val base=BuildConfig.SOCIAL_MEDIA_URL.trimEnd('/');if(base.isNotBlank()){val rb=object:RequestBody(){override fun contentType()=mime.toMediaType();override fun contentLength()=size;override fun writeTo(sink:BufferedSink){app.contentResolver.openInputStream(uri)?.use{input->input.copyTo(sink.outputStream())}?:throw IOException("Cannot open media")}};val req=Request.Builder().url("$base/v1/media?kind=${enc(kind)}").header("Authorization","Bearer $token").header("Content-Type",mime).header("Content-Length",size.toString()).post(rb).build();http.newCall(req).enqueue(object:Callback{override fun onFailure(call:Call,e:IOException){callback.onComplete(SocialResult(error=e))};override fun onResponse(call:Call,response:Response){response.use{val text=it.body?.string().orEmpty();if(!it.isSuccessful)callback.onComplete(SocialResult(error=IOException("Media Worker ${it.code}: ${text.take(300)}")))else try{val o=gson.fromJson(text,JsonObject::class.java);val url=o.get("url")?.takeUnless{x->x.isJsonNull}?.asString;callback.onComplete(if(!url.isNullOrBlank())SocialResult(value=url)else SocialResult(error=IOException("Media Worker returned no URL")))}catch(t:Throwable){callback.onComplete(SocialResult(error=t))}}}})}else{val ext=mime.substringAfter('/','bin').substringBefore(';').replace(Regex("[^A-Za-z0-9]"),"");val file="${UUID.randomUUID()}.$ext";val path="$kind/$uid/$file";val rb=object:RequestBody(){override fun contentType()=mime.toMediaType();override fun contentLength()=size;override fun writeTo(sink:BufferedSink){app.contentResolver.openInputStream(uri)?.use{it.copyTo(sink.outputStream())}?:throw IOException("Cannot open media")}};val req=Request.Builder().url(BuildConfig.SUPABASE_URL.trimEnd('/')+"/storage/v1/object/tv49-profile-media/$path").header("apikey",BuildConfig.SUPABASE_ANON_KEY).header("Authorization","Bearer $token").header("x-upsert","false").put(rb).build();http.newCall(req).enqueue(object:Callback{override fun onFailure(call:Call,e:IOException){callback.onComplete(SocialResult(error=e))};override fun onResponse(call:Call,response:Response){response.use{if(!it.isSuccessful)callback.onComplete(SocialResult(error=IOException("Storage ${it.code}: ${it.body?.string()?.take(300)}")))else callback.onComplete(SocialResult(value=publicMediaUrl(kind,uid,file)))}}})}}
