...
class SupabaseSocialRepository(context: Context) {
    ...
    /**
     * Uploads binary media directly to the authenticated Cloudflare R2 Worker.
     * Supabase stores only post/profile metadata and the resulting Cloudflare URL.
     */
    fun uploadMedia(uri:Uri,kind:String,callback:ResultCallback<String>){
        ...
        val normalizedKind=when(kind.lowercase()){
            "post-image","post-video","posts"->"posts"
            "avatar","avatars"->"avatars"
            "cover","covers"->"covers"
            else->return fail(callback,"Unsupported media destination")
        }
