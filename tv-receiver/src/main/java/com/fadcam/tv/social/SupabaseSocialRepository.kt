...
class SupabaseSocialRepository(context: Context) {
    ...
    fun signIn(email: String, password: String, callback: ResultCallback<SocialSession>) {
        ...
        request("/auth/v1/token?grant_type=password", "POST", body(mapOf("email" to email.trim(), "password" to password)), null, null) { r ->
            completeSession(r, callback)
        }
    }
    ...
    fun signUp(email: String, password: String, username: String, displayName: String, callback: ResultCallback<SocialSession>) {
        ...
        val data = mapOf("username" to u, "display_name" to displayName.trim())
        request("/auth/v1/signup", "POST", body(mapOf("email" to email.trim(), "password" to password, "data" to data)), null, null) { r ->
            ...
            try {
                ...
                if (o.get("access_token") == null || o.get("access_token").isJsonNull || o.get("access_token").asString.isBlank()) {
                    callback.onComplete(SocialResult(error = IllegalStateException("Account created. Confirm the email before signing in.")))
                } else completeSession(r, callback)
    ...
    fun refreshSession(callback: ResultCallback<SocialSession>) {
        val token = currentRefreshToken() ?: return fail(callback, "No refresh token")
        request("/auth/v1/token?grant_type=refresh_token", "POST", body(mapOf("refresh_token" to token)), null, null) { r -> completeSession(r, callback) }
    }
    ...
    fun updateProfile(username: String, displayName: String, bio: String, avatarUrl: String?, callback: ResultCallback<SocialUser?>) {
        ...
        mutate("/rest/v1/profiles?id=eq.${enc(id)}&select=id,username,display_name,avatar_url,bio", "PATCH", body(fields), callback) { parseProfiles(it).firstOrNull() }
    }
    ...
    fun replyToPost(postId: String, bodyText: String, callback: ResultCallback<Boolean>) {
        ...
        raw("/rest/v1/post_replies", "POST", body(mapOf("post_id" to postId, "author_id" to uid, "body" to text)), callback)
    }
    ...
    fun markNotificationRead(id: String, callback: ResultCallback<Boolean>) =
        raw("/rest/v1/notifications?id=eq.${enc(id)}", "PATCH", body(mapOf("read_at" to "now()")), callback)
    ...
    fun sendMessage(conversationId: String, bodyText: String, callback: ResultCallback<Boolean>) {
        ...
        raw("/rest/v1/messages", "POST", body(mapOf("conversation_id" to conversationId, "sender_id" to uid, "body" to text)), callback)
    }

    fun loadMessages(conversationId: String, limit: Int = 50, callback: ResultCallback<String>) =
        getText("/rest/v1/messages?conversation_id=eq.${enc(conversationId)}&select=id,sender_id,body,media_url,media_type,created_at,edited_at&order=created_at.desc&limit=${limit.coerceIn(1,100)}", callback)

    /** Streaming upload with a hard 20 MiB limit. Kept on Supabase until the R2 Worker is deployed. */
    fun uploadMedia(uri: Uri, kind: String, callback: ResultCallback<String>) {
        ...
        val requestBody = object : RequestBody() {
            ...
            override fun writeTo(sink: BufferedSink) {
                app.contentResolver.openInputStream(uri)?.use { input -> input.copyTo(sink.outputStream()) } ?: throw IOException("Cannot open media")
            }
        }
        ...
        http.newCall(request).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                response.use { if (!it.isSuccessful) callback.onComplete(SocialResult(error = IOException("Storage ${it.code}: ${it.body?.string()?.take(300)}"))) else callback.onComplete(SocialResult(value = publicMediaUrl(kind, uid, file))) }
            }
        })
    }
    ...
    fun attachMedia(postId: String, url: String, mediaType: String, mimeType: String?, size: Long, order: Int, callback: ResultCallback<Boolean>) {
        ...
        raw("/rest/v1/post_media", "POST", body(mapOf("post_id" to postId, "owner_id" to uid, "media_url" to url, "media_type" to mediaType, "mime_type" to mimeType, "byte_size" to size, "sort_order" to order)), callback)
    }
    ...
    private fun postsPath(limit: Int): String = "/rest/v1/posts?select=id,body,media_url,media_type,created_at,like_count,reply_count,repost_count,author:profiles!posts_author_id_fkey(id,username,display_name,avatar_url,bio)&order=created_at.desc&limit=${limit.coerceIn(1,50)}"
    ...
    private fun toggle(table: String, key: String, value: String, userKey: String, enabled: Boolean, callback: ResultCallback<Boolean>) {
        ...
        if (enabled) raw("/rest/v1/$table", "POST", body(mapOf(key to value, userKey to uid)), callback)
    ...
    private fun <T> get(path: String, callback: ResultCallback<T>, parser: (String) -> T) {
        request(path, "GET", null, currentAccessToken(), null) { r ->
            if (r.error != null) callback.onComplete(SocialResult(error = r.error)) else try { callback.onComplete(SocialResult(value = parser(r.value))) } catch (t: Throwable) { callback.onComplete(SocialResult(error = t)) }
        }
    }
    ...
    private fun request(path: String, method: String, payload: String?, token: String?, prefer: String?, callback: (RawResult) -> Unit) {
        ...
        val builder = Request.Builder().url(url).header("apikey", BuildConfig.SUPABASE_ANON_KEY)
        if (!token.isNullOrBlank()) builder.header("Authorization", "Bearer $token")
        if (!prefer.isNullOrBlank()) builder.header("Prefer", prefer)
        if (payload != null) builder.method(method, payload.toRequestBody(jsonType)) else builder.method(method, null)
        http.newCall(builder.build()).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) { callback(RawResult("", e)) }
            override fun onResponse(call: Call, response: Response) { response.use { val text = it.body?.string().orEmpty(); callback(if (it.isSuccessful) RawResult(text, null) else RawResult(text, IOException("Supabase ${it.code}: ${text.take(300)}"))) } }
        })
    }

    private fun completeSession(r: RawResult, callback: ResultCallback<SocialSession>) {
        if (r.error != null) return callback.onComplete(SocialResult(error = r.error))
        try { callback.onComplete(SocialResult(value = parseAndSaveSession(r.value))) } catch (t: Throwable) { callback.onComplete(SocialResult(error = t)) }
    }
    ...
    private fun parseAndSaveSession(text: String): SocialSession {
        ...
        prefs.edit().putString("access_token", access).apply { if (refresh != null) putString("refresh_token", refresh); if (uid != null) putString("user_id", uid) }.apply()
        return session
    }

    private fun parseProfiles(text: String): List<SocialUser> {
        val type = object : TypeToken<List<RemoteProfile>>() {}.type
        return (gson.fromJson<List<RemoteProfile>>(text, type) ?: emptyList()).map { SocialUser(it.id, it.username.orEmpty(), it.displayName.orEmpty(), it.avatarUrl, it.bio) }
    }

    private fun parsePosts(text: String): List<SocialPost> {
        val type = object : TypeToken<List<RemotePost>>() {}.type
        return (gson.fromJson<List<RemotePost>>(text, type) ?: emptyList()).map { p -> SocialPost(p.id, p.author?.let { SocialUser(it.id, it.username.orEmpty(), it.displayName.orEmpty(), it.avatarUrl, it.bio) } ?: SocialUser("", "", "TV 49 East user"), p.body.orEmpty(), p.mediaUrl, p.mediaType, p.createdAt.orEmpty(), p.likeCount, p.replyCount, p.repostCount) }
    }

    private fun body(value: Any): String = gson.toJson(value)
