...
public final class ReelRanker {
    ...
    private static double score(SocialPost p) {
        ...
        double engagement = Math.log1p(Math.max(0,p.getLikeCount()))*1.2 + Math.log1p(Math.max(0,p.getReplyCount()))*1.4 + Math.log1p(Math.max(0,p.getRepostCount()))*1.8;
        double media = "video".equalsIgnoreCase(p.getMediaType()) ? 5.0 : 0.0;
        double affinity = p.getLikedByViewer() ? 2.0 : 0.0;
        return engagement + media + affinity + 1.0;
    }
}
