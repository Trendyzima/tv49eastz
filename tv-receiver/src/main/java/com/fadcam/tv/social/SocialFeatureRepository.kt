...
class SocialFeatureRepository(context: Context) {
    ...
    fun quotePost(postId: String, comment: String, callback: Callback<Boolean>) {
        ...
        upsert("/rest/v1/post_quotes?on_conflict=author_id,post_id", mapOf("author_id" to uid, "post_id" to postId, "comment" to body), callback)
    }

    fun viewPost(postId: String, callback: Callback<Boolean>) {
        if (userId() == null) return fail(callback, "Sign in required")
        if (postId.isBlank()) return fail(callback, "Post id required")
        rpc("record_post_view", mapOf("p_post_id" to postId)) { result -> callback.onComplete(if (result.error == null) SocialResult(value = true) else SocialResult(error = result.error)) }
    }
    ...
    fun toggleListFollow(listId: String, enabled: Boolean, callback: Callback<Boolean>) {
        ...
        if (enabled) upsert("/rest/v1/list_followers?on_conflict=list_id,user_id", mapOf("list_id" to listId, "user_id" to uid), callback) else delete("/rest/v1/list_followers?$filter", callback)
    }
    ...
    fun createList(name: String, description: String, privateList: Boolean, callback: Callback<String?>) {
        ...
        postReturning("/rest/v1/lists?select=id", mapOf("owner_id" to uid, "name" to clean, "description" to description.trim().take(500), "is_private" to privateList), callback) { raw -> SocialResult(value = parseId(raw)) }
    }

    fun addListMember(listId: String, memberId: String, enabled: Boolean, callback: Callback<Boolean>) {
        if (userId() == null) return fail(callback, "Sign in required")
        if (enabled) upsert("/rest/v1/list_members?on_conflict=list_id,user_id", mapOf("list_id" to listId, "user_id" to memberId), callback) else delete("/rest/v1/list_members?list_id=eq.${enc(listId)}&user_id=eq.${enc(memberId)}", callback)
    }
    ...
    fun createConversation(memberIds: List<String>, callback: Callback<String?>) {
        ...
        rpc("create_conversation_atomic", mapOf("p_member_ids" to members)) { result ->
            if (result.error != null) callback.onComplete(SocialResult(error = result.error)) else callback.onComplete(SocialResult(value = parseScalarId(result.value.orEmpty())))
        }
    }
    ...
    fun sendMessage(conversationId: String, body: String, replyToMessageId: String? = null, sharedPostId: String? = null, clientMessageId: String = UUID.randomUUID().toString(), callback: Callback<String?>) {
        ...
        rpc("send_message_idempotent", mapOf("p_conversation_id" to conversationId, "p_body" to clean, "p_reply_to_message_id" to replyToMessageId, "p_shared_post_id" to sharedPostId, "p_client_message_id" to clientMessageId)) { result ->
            if (result.error != null) callback.onComplete(SocialResult(error = result.error)) else callback.onComplete(SocialResult(value = parseScalarId(result.value.orEmpty())))
        }
    }
    ...
    fun reactToMessage(messageId: String, reaction: String, enabled: Boolean, callback: Callback<Boolean>) {
        ...
        if (enabled) upsert("/rest/v1/message_reactions?on_conflict=message_id,user_id,reaction", mapOf("message_id" to messageId, "user_id" to uid, "reaction" to clean), callback) else delete("/rest/v1/message_reactions?$filter", callback)
    }
    ...
    fun editMessage(messageId: String, body: String, callback: Callback<Boolean>) {
        ...
        patch("/rest/v1/messages?id=eq.${enc(messageId)}", mapOf("body" to clean, "edited_at" to nowIso()), callback)
    }
    ...
    fun saveDraft(draftId: String?, body: String, metadata: String = "{}", callback: Callback<String?>) {
        ...
        else patchRaw("/rest/v1/post_drafts?id=eq.${enc(draftId)}", mapOf("body" to clean, "metadata" to parsedMetadata, "updated_at" to nowIso())) { result -> callback.onComplete(if (result.error == null) SocialResult<String?>(value = draftId) else SocialResult(error = result.error)) }
    }
    ...
    fun editPost(postId: String, body: String, callback: Callback<Boolean>) {
        ...
        patch("/rest/v1/posts?id=eq.${enc(postId)}", mapOf("body" to clean, "edited_at" to nowIso()), callback)
    }

    fun loadConversations(limit: Int = 50, callback: Callback<String>) {
        if (userId() == null) return fail(callback, "Sign in required")
        rpc("get_conversations_with_latest", mapOf("p_limit" to limit.coerceIn(1, 100))) { result -> callback.onComplete(if (result.error == null) SocialResult(value = result.value) else SocialResult(error = result.error)) }
    }
    ...
    fun loadListTimeline(listId: String, limit: Int = 30, offset: Int = 0, callback: Callback<String>) {
        if (userId() == null) return fail(callback, "Sign in required")
        rpc("get_list_timeline", mapOf("p_list_id" to listId, "p_limit" to limit.coerceIn(1, 100), "p_offset" to offset.coerceAtLeast(0))) { result -> callback.onComplete(if (result.error == null) SocialResult(value = result.value) else SocialResult(error = result.error)) }
    }

    fun requestFollow(targetUserId: String, callback: Callback<Boolean>) {
        val uid = userId() ?: return fail(callback, "Sign in required")
        if (uid == targetUserId) return fail(callback, "You cannot follow yourself")
        upsert("/rest/v1/follow_requests?on_conflict=requester_id,target_id", mapOf("requester_id" to uid, "target_id" to targetUserId), callback)
    }

    fun cancelFollowRequest(targetUserId: String, callback: Callback<Boolean>) {
        val uid = userId() ?: return fail(callback, "Sign in required")
        delete("/rest/v1/follow_requests?requester_id=eq.${enc(uid)}&target_id=eq.${enc(targetUserId)}", callback)
    }

    fun respondToFollowRequest(requesterId: String, accept: Boolean, callback: Callback<Boolean>) {
        val uid = userId() ?: return fail(callback, "Sign in required")
        patch("/rest/v1/follow_requests?requester_id=eq.${enc(requesterId)}&target_id=eq.${enc(uid)}", mapOf("status" to if (accept) "accepted" else "declined"), callback)
    }

    fun markMessageRead(messageId: String, callback: Callback<Boolean>) = patch("/rest/v1/messages?id=eq.${enc(messageId)}", mapOf("read_at" to nowIso()), callback)
    fun markMessageDelivered(messageId: String, callback: Callback<Boolean>) = patch("/rest/v1/messages?id=eq.${enc(messageId)}", mapOf("delivered_at" to nowIso()), callback)
    ...
    private fun toggleRelation(table: String, actorColumn: String, targetColumn: String, target: String, enabled: Boolean, callback: Callback<Boolean>) {
        ...
        if (enabled) upsert("/rest/v1/$table?on_conflict=$actorColumn,$targetColumn", mapOf(actorColumn to uid, targetColumn to target), callback) else delete("/rest/v1/$table?$filter", callback)
    }

    private fun write(path: String, fields: Map<String, Any?>, callback: Callback<Boolean>) = postRaw(path, fields, null) { result -> callback.onComplete(if (result.error == null) SocialResult(value = true) else SocialResult(error = result.error)) }
    private fun upsert(path: String, fields: Map<String, Any?>, callback: Callback<Boolean>) = postRaw(path, fields, "resolution=ignore-duplicates,return=minimal") { result -> callback.onComplete(if (result.error == null) SocialResult(value = true) else SocialResult(error = result.error)) }
    private fun postReturning(path: String, fields: Map<String, Any?>, callback: Callback<String?>, parser: (String) -> SocialResult<String?>) = postRaw(path, fields, "return=representation") { result -> if (result.error != null) callback.onComplete(SocialResult(error = result.error)) else callback.onComplete(parser(result.value.orEmpty())) }
    ...
    private fun delete(path: String, callback: Callback<Boolean>) = executeText(requestBuilder(path, "return=minimal").delete().build()) { result -> callback.onComplete(if (result.error == null) SocialResult(value = true) else SocialResult(error = result.error)) }
    private fun get(path: String, callback: Callback<String>) = executeText(requestBuilder(path, null).get().build()) { result -> callback.onComplete(if (result.error == null) SocialResult(value = result.value) else SocialResult(error = result.error)) }
