...
class SocialFeatureRepository(context: Context) {
    ...
    fun createList(name: String, description: String, privateList: Boolean, callback: Callback<String?>) {
        ...
        postReturning("/rest/v1/lists?select=id", mapOf("owner_id" to uid, "name" to clean, "description" to description.trim().take(500), "is_private" to privateList), callback) { parseId(it) }
    }
    ...
    fun sendMessage(conversationId: String, body: String, replyToMessageId: String? = null, sharedPostId: String? = null, clientMessageId: String = UUID.randomUUID().toString(), callback: Callback<String?>) {
        ...
        postReturning("/rest/v1/messages?select=id", fields, callback) { parseId(it) }
    }
