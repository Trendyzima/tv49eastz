...
class SocialFeatureRepository(context: Context) {
    ...
    // Read-only native parity surfaces. Each method talks to Supabase/PostgREST and returns
    // server JSON unchanged so the UI never invents social state locally.
    fun loadNotifications(limit:Int=50, callback:Callback<String>)=loadRows("notifications?select=*&order=created_at.desc&limit=${limit.coerceIn(1,100)}",callback)
    fun loadBookmarks(limit:Int=50, callback:Callback<String>) { val uid=userId()?:return fail(callback,"Sign in required"); loadRows("bookmarks?select=*&user_id=eq.${enc(uid)}&order=created_at.desc&limit=${limit.coerceIn(1,100)}",callback) }
    fun loadHistory(limit:Int=50, callback:Callback<String>) { val uid=userId()?:return fail(callback,"Sign in required"); loadRows("post_views?select=*&user_id=eq.${enc(uid)}&order=created_at.desc&limit=${limit.coerceIn(1,100)}",callback) }
    fun loadStories(limit:Int=50, callback:Callback<String>) { if(userId()==null)return fail(callback,"Sign in required"); loadRows("stories?select=*&order=created_at.desc&limit=${limit.coerceIn(1,100)}",callback) }
    fun loadCommunities(limit:Int=50, callback:Callback<String>) { if(userId()==null)return fail(callback,"Sign in required"); loadRows("communities?select=*&order=created_at.desc&limit=${limit.coerceIn(1,100)}",callback) }
    fun loadPolls(limit:Int=50, callback:Callback<String>) { if(userId()==null)return fail(callback,"Sign in required"); loadRows("polls?select=*&order=created_at.desc&limit=${limit.coerceIn(1,100)}",callback) }
    fun loadRows(path:String, callback:Callback<String>) { if(userId()==null)return fail(callback,"Sign in required"); get("/rest/v1/$path",callback) }
    ...
    private fun loadRows(path:String,callback:Callback<String>)=get("/rest/v1/$path",callback)
