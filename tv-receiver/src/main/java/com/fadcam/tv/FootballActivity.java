...
public final class FootballActivity extends Activity {
    ...
    private Button button(String value, View.OnClickListener listener) {
        Button b = new Button(this); b.setText(value); b.setTextSize(13); b.setAllCaps(false); b.setOnClickListener(listener); b.setFocusable(true); return b;
    }
    ...
    private void buildUi() {
        ...
        Button refresh = button("Refresh", v -> load()); header.addView(refresh, new LinearLayout.LayoutParams(dp(100), dp(46))); root.addView(header);

        HorizontalScrollView competitions = new HorizontalScrollView(this); competitions.setHorizontalScrollBarEnabled(false);
        LinearLayout c = new LinearLayout(this); String[] leagues = {"Premier League","LaLiga","Bundesliga","Serie A","Ligue 1","UEFA Champions League","MLS"};
    ...
    private void load() {
        ...
        repository.loadMatches(competition, mode, (matches, error) -> runOnUiThread(() -> {
            ...
            int index = 1;
            for (FootballMatch match : matches) addMatch(match, index++);
        }));
    }
    ...
    private void addMatch(FootballMatch m, int index) {
        ...
        card.setBackgroundColor(Color.rgb(248,250,251));
        if ("table".equals(m.state)) {
            TextView row = text(index + ".  " + m.away + "    " + m.status, 15, Color.rgb(15,20,25), true); row.setPadding(0, dp(6), 0, dp(6)); card.addView(row);
        } else {
            TextView comp = text(m.competition + "  •  " + m.status, 11, Color.DKGRAY, false); card.addView(comp);
            TextView teams = text(m.home + "    " + m.homeScore + "  –  " + m.awayScore + "    " + m.away, 16, Color.rgb(15,20,25), true); teams.setPadding(0, dp(5), 0, dp(2)); card.addView(teams);
            TextView kick = text(m.kickoff, 11, Color.DKGRAY, false); card.addView(kick);
        }
