...
export class SessionStore extends DurableObject {
  async fetch(request: Request): Promise<Response> {
    ...
    if (request.method === "GET" && new URL(request.url).pathname === "/verify") {
      ...
      if (parts.length !== 2) return new Response("invalid", { status: 401 });
      try {
        const payload = new TextDecoder().decode(base64UrlDecode(parts[0]));
        const fields = payload.split("\x00");
        if (fields.length !== 4 || fields[0] !== "gateway" || fields[1] !== record.session || fields[2] !== record.stream || Number(fields[3]) !== record.exp) {
          return new Response("invalid", { status: 401 });
        }
        const expected = await hmac((this.env as any).GATEWAY_CAPABILITY_KEY, payload);
