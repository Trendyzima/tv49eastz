...
const SUPABASE_URL = "https://vfhehknmxxedvesdvpew.supabase.co";
...
async function createProducerSession(request: Request, env: Env): Promise<Response> {
  ...
  const saved = await env.SESSION_STORE.get(env.SESSION_STORE.idFromName(session)).fetch(new Request("https://session/store", {
    method: "POST",
...
function appendTicket(uri: string, ticket: string): string {
  ...
  return `${uri}${separator}ticket=${encodeURIComponent(ticket)}`;
}
