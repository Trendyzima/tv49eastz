...
async function authenticate(request: Request, env: MediaEnv): Promise<string | null> {
  ...
  try { const r = await fetch(`${env.SUPABASE_URL.replace(/\/$/, "")}/auth/v1/user`, { headers: { Authorization: `Bearer ${token}`, apikey: env.SUPABASE_PUBLISHABLE_KEY } }); if (!r.ok) return null; const u = await r.json() as { id?: string }; return u.id && /^[0-9a-f-]{36}$/i.test(u.id) ? u.id : null; } catch { return null; }
}
async function upload(request: Request, env: MediaEnv): Promise<Response> {
  const userId = await authenticate(request, env); if (!userId) return json({ error: "unauthorized" }, 401);
  const type = (request.headers.get("content-type") || "").split(";", 1)[0].toLowerCase(); const size = Number(request.headers.get("content-length") || "-1");
  ...
  const key = `social/${userId}/${crypto.randomUUID()}.${ext}`;
  ...
  return json({ key, url: `${new URL(request.url).origin}/media/${key}`, media_type: type.startsWith("video/") ? "video" : "image", byte_size: size });
}
async function download(key: string, env: MediaEnv): Promise<Response> {
  ...
  const object = await env.SOCIAL_MEDIA.get(key); if (!object) return new Response("Not found", { status: 404 });
