...
export async function handlePayPalRequest(
  ...
  if (captureMatch && request.method === "POST") {
    return captureOrder(captureMatch[1], request, env);
  }
  ...
  if (url.pathname === "/v1/paypal/webhook" && request.method === "POST") {
...
async function createOrder(request: Request, env: PayPalEnv): Promise<Response> {
  ...
  const applicationContext: JsonRecord = {
  ...
  const response = await paypalFetch("/v2/checkout/orders", accessToken, {
    method: "POST",
...
async function captureOrder(
  ...
  const response = await paypalFetch(`/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`, accessToken, {
    method: "POST",
...
async function showOrder(orderId: string, env: PayPalEnv): Promise<Response> {
  ...
  const accessToken = await getAccessToken(env);
  if (!accessToken) return json({ error: "paypal_not_configured" }, 503);
  const response = await paypalFetch(`/v2/checkout/orders/${encodeURIComponent(orderId)}`, accessToken);
  return paypalResponse(response);
}
...
async function verifyWebhook(request: Request, env: PayPalEnv): Promise<Response> {
  ...
  const verifyResponse = await paypalFetch("/v1/notifications/verify-webhook-signature", accessToken, {
    method: "POST",
    body: JSON.stringify({
      auth_algo: authAlgo,
      cert_url: certUrl,
  ...
  if (verification.verification_status !== "SUCCESS") {
    return json({ error: "paypal_webhook_signature_invalid" }, 400);
  }
  ...
  // Payment fulfillment must be idempotent. The event ID is returned to the
  // caller so the application can persist/ignore duplicates in its database.
  return json({ ok: true, verified: true, event_id: eventId, event_type: eventType });
}
...
async function paypalFetch(path: string, accessToken: string, init: RequestInit = {}): Promise<Response> {
  const headers = new Headers(init.headers);
  headers.set("Authorization", `Bearer ${accessToken}`);
  headers.set("Accept", "application/json");
  if (init.body && !headers.has("Content-Type")) headers.set("Content-Type", "application/json");
  return fetch(`${currentBaseUrl}${path}`, { ...init, headers });
}

// Set immediately before each PayPal call so the helper remains pure with
// respect to the Worker environment while retaining one request implementation.
let currentBaseUrl = "https://api-m.sandbox.paypal.com";
