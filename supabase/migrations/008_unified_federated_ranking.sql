...
create table if not exists public.federated_object_reactions (
  id uuid primary key default gen_random_uuid(), object_uri text not null references public.federated_objects(uri) on delete cascade, user_id uuid not null references public.profiles(id) on delete cascade,
  reaction_type text not null check(reaction_type in ('like','repost','save')), activity_uri text unique, active boolean not null default true,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique(object_uri,user_id,reaction_type)
);
...
create table if not exists public.federated_follow_requests (
  id uuid primary key default gen_random_uuid(), local_user_id uuid not null references public.profiles(id) on delete cascade,
  remote_actor_uri text not null references public.federated_actors(uri) on delete cascade, activity_uri text unique,
  state text not null default 'pending' check(state in ('pending','active','accepted','rejected','removed')),
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique(local_user_id,remote_actor_uri)
);
...
create table if not exists public.federated_feed_events (
  ...
  dwell_ms bigint not null default 0 check(dwell_ms>=0), created_at timestamptz not null default now()
);
...
local_base as (
 select 'local'::text source,('local:'||p.id::text) object_key,p.author_id,null::text author_uri,pf.username author_username,pf.display_name author_display_name,pf.avatar_url author_avatar_url,p.body,p.media_url,p.media_type,p.created_at,p.like_count::bigint like_count,p.reply_count::bigint reply_count,p.repost_count::bigint repost_count,
  (greatest(0,1.0-extract(epoch from(now()-p.created_at))/172800.0)*3.0)+ln(1+p.like_count)*1.15+ln(1+p.reply_count)*1.45+ln(1+p.repost_count)*1.9+case when exists(select 1 from public.follows f where f.follower_id=auth.uid() and f.following_id=p.author_id) then 5 else 0 end+case when exists(select 1 from public.content_events e where e.user_id=auth.uid() and e.post_id=p.id and e.event_type in('open','like','reply','repost','save')) then 4 else 0 end-case when exists(select 1 from public.content_events e where e.user_id=auth.uid() and e.post_id=p.id and e.event_type in('hide','not_interested')) then 15 else 0 end+case when p.created_at>now()-interval '15 minutes' then 2.5 else 0 end score
 from public.posts p join public.profiles pf on pf.id=p.author_id
),
remote_base as (
 select 'federated'::text source,('remote:'||fo.uri) object_key,null::uuid author_id,fo.actor_uri,coalesce(fa.preferred_username,'remote') author_username,coalesce(fa.display_name,fa.preferred_username,'Federated user') author_display_name,fa.avatar_url author_avatar_url,fo.content body,coalesce(fo.attachments->0->>'url',fo.url) media_url,case when lower(coalesce(fo.attachments->0->>'type',''))='video' then 'video' when lower(coalesce(fo.attachments->0->>'type',''))='image' then 'image' else null end media_type,coalesce(fo.published_at,fo.created_at) created_at,
  ((select count(*) from public.federated_object_reactions r where r.object_uri=fo.uri and r.reaction_type='like' and r.active)+(select count(*) from public.federated_activities a where a.object_uri=fo.uri and a.activity_type='Like'))::bigint like_count,
  ...
  ((select count(*) from public.federated_object_reactions r where r.object_uri=fo.uri and r.reaction_type='repost' and r.active)+(select count(*) from public.federated_activities a where a.object_uri=fo.uri and a.activity_type='Announce'))::bigint repost_count,
  (greatest(0,1.0-extract(epoch from(now()-coalesce(fo.published_at,fo.created_at)))/259200.0)*2.8)+ln(1+(select count(*) from public.federated_activities a where a.object_uri=fo.uri and a.activity_type='Like'))*1.1+ln(1+(select count(*) from public.federated_activities a where a.object_uri=fo.uri and a.activity_type='Announce'))*1.8+ln(1+(select count(*) from public.federated_activities a where a.object_uri=fo.uri and a.activity_type in('Create','Reply')))*1.25+case when exists(select 1 from public.federated_follow_requests fr where fr.local_user_id=auth.uid() and fr.remote_actor_uri=fo.actor_uri and fr.state in('active','accepted')) then 5 else 0 end+case when exists(select 1 from public.federated_feed_events e where e.user_id=auth.uid() and e.object_uri=fo.uri and e.event_type in('open','like','reply','repost','save')) then 4 else 0 end-case when exists(select 1 from public.federated_feed_events e where e.user_id=auth.uid() and e.object_uri=fo.uri and e.event_type in('hide','not_interested')) then 15 else 0 end+case when coalesce(fo.published_at,fo.created_at)>now()-interval '30 minutes' then 2 else 0 end score
 from public.federated_objects fo left join public.federated_actors fa on fa.uri=fo.actor_uri where not fo.sensitive and fo.object_type in('Note','Article','Image','Video','Page','Question','Event')
),
candidates as (
...
filtered as (select c.*,row_number() over(partition by coalesce(c.author_id::text,c.author_uri) order by c.score desc,c.created_at desc) author_rank,row_number() over(partition by coalesce(split_part(c.author_uri,'/',3),'local') order by c.score desc,c.created_at desc) domain_rank from candidates c),
...
select source,object_key,author_id,author_uri,author_username,author_display_name,author_avatar_url,body,media_url,media_type,created_at,like_count,reply_count,repost_count,exploration_score score from explore order by score desc,created_at desc limit (select lim from params) offset (select off from params); $$;
...
returns jsonb language plpgsql security definer set search_path=public as $$ declare v_id uuid; begin
 if auth.uid() is null then raise exception 'unauthorized'; end if; if not exists(select 1 from public.federated_objects where uri=p_object_uri) then raise exception 'object_not_found'; end if;
 insert into public.federated_object_reactions(object_uri,user_id,reaction_type,active,updated_at) values(p_object_uri,auth.uid(),p_reaction_type,p_active,now()) on conflict(object_uri,user_id,reaction_type) do update set active=excluded.active,updated_at=now() returning id into v_id;
 return jsonb_build_object('id',v_id,'object_uri',p_object_uri,'reaction_type',p_reaction_type,'active',p_active); end; $$;
...
returns jsonb language plpgsql security definer set search_path=public as $$ declare v_id uuid; begin
 if auth.uid() is null then raise exception 'unauthorized'; end if; if not exists(select 1 from public.federated_actors where uri=p_remote_actor_uri) then raise exception 'actor_not_found'; end if;
 insert into public.federated_follow_requests(local_user_id,remote_actor_uri,state,updated_at) values(auth.uid(),p_remote_actor_uri,case when p_active then 'pending' else 'removed' end,now()) on conflict(local_user_id,remote_actor_uri) do update set state=case when p_active then 'pending' else 'removed' end,updated_at=now() returning id into v_id;
 return jsonb_build_object('id',v_id,'remote_actor_uri',p_remote_actor_uri,'active',p_active); end; $$;
