...
func acceptDevices(ln net.Listener,p *pool,expected string){
    for { c,err:=ln.Accept(); if err!=nil{time.Sleep(time.Second);continue}; go register(c,p,expected) }
}

func register(c net.Conn,p *pool,expected string){
    defer func(){ if r:=recover(); r!=nil { _=c.Close() } }()
    ...
    // Device identity is taken from the verified client certificate, not from
    // an HTTP header. The application-level hello must match it exactly.
    certID:=strings.TrimSpace(cert.Subject.CommonName)
    if certID=="" || certID!=expected { _=c.Close(); return }
    br:=bufio.NewReaderSize(c,256); hello,err:=br.ReadString('\n'); if err!=nil { _=c.Close(); return }
    hello=strings.TrimSpace(hello); want:="TV49-TUNNEL/1 "+certID; if hello!=want { _=c.Close(); return }
...
func serve(client net.Conn,p *pool){
    ...
    defer tunnel.Close()
    _=tunnel.SetWriteDeadline(time.Now().Add(5*time.Second)); if _,err=tunnel.Write([]byte("START\n"));err!=nil{return}
    _=tunnel.SetDeadline(time.Time{}); proxy(client,tunnel)
}
