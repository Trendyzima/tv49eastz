...
func maintain(cfg *tls.Config, gateway, local string) {
    for {
        if err := connect(cfg, gateway, local); err != nil { time.Sleep(2 * time.Second) }
    }
}
...
func connect(cfg *tls.Config, gateway, local string) error {
    ...
    // The gateway keeps this mTLS connection idle until it assigns it to a
    // single local TCP request. No local server connection is opened early.
    command := make([]byte, 6)
