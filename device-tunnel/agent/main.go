...
func tlsConfig() (*tls.Config, error) {
    ...
    cert, err := tls.LoadX509KeyPair(env("TUNNEL_CERT", "device.pem"), env("TUNNEL_KEY", "device-key.pem")); if err != nil { return nil, err }
...
func main() {
    pool, err := strconv.Atoi(env("TUNNEL_POOL", "8")); if err != nil || pool < 1 || pool > 32 { pool = 8 }
    deviceID := strings.TrimSpace(os.Getenv("TUNNEL_DEVICE_ID")); if deviceID == "" { panic("TUNNEL_DEVICE_ID is required") }
...
func connect(cfg *tls.Config, gateway, local, deviceID string) error {
    conn, err := tls.DialWithDialer(&net.Dialer{Timeout: 8 * time.Second, KeepAlive: 30 * time.Second}, "tcp", gateway, cfg); if err != nil { return err }
    defer conn.Close()
    hello := "TV49-TUNNEL/1 " + deviceID + "\n"; if _, err = conn.Write([]byte(hello)); err != nil { return err }
    ack := make([]byte, 3); if _, err = io.ReadFull(conn, ack); err != nil { return err }; if string(ack) != "OK\n" { return fmt.Errorf("gateway rejected tunnel") }
    command := make([]byte, 6); if _, err = io.ReadFull(conn, command); err != nil { return err }; if string(command) != "START\n" { return fmt.Errorf("unexpected tunnel command") }
    upstream, err := net.DialTimeout("tcp", local, 5*time.Second); if err != nil { return err }; defer upstream.Close()
