...
public final class FadCamTvPublisher {
    ...
    public static String provision(@NonNull Context context, @NonNull String deviceId,
        ...
        ensureKey();
        String publicKey = getPublisherPublicKeyBase64();
        ...
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().putString(PROVISIONED_DEVICE, id).apply();
        return fingerprint;
    }
    ...
    public static String startTv(@NonNull Context context, @NonNull String deviceId,
        ...
        ensureKey();
        String publicKey = getPublisherPublicKeyBase64();
        PublisherSession session = requestGatewaySession(id, channelId, streamId, publicKey);
        String handoff = buildHandoffUri(session.playlist, name == null ? "FadCam Local" : name,
                owner == null ? "FadCam" : owner, publicKey).toString();
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().putString(ACTIVE_SESSION, session.sessionId).apply();
        return handoff;
    }
    ...
    public static void stopTv(@NonNull Context context) throws Exception {
        String sessionId = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).getString(ACTIVE_SESSION, "");
    ...
    public static String getPublisherPublicKeyBase64() throws Exception {
        return Base64.encodeToString(loadPublicKey().getEncoded(), Base64.URL_SAFE | Base64.NO_WRAP | Base64.NO_PADDING);
    }

    private static PublisherSession requestGatewaySession(String deviceId, String channelId, String streamId, String publicKey) throws Exception {
    ...
    private static void revokeSession(String sessionId) throws Exception {
        ...
        try {
            ...
            connection = (HttpURLConnection) url.openConnection(); connection.setRequestMethod("DELETE");
            connection.setConnectTimeout(CONNECT_TIMEOUT_MS); connection.setReadTimeout(READ_TIMEOUT_MS);
            int status = connection.getResponseCode();
            if (status != HttpURLConnection.HTTP_NO_CONTENT && status != HttpURLConnection.HTTP_OK && status != HttpURLConnection.HTTP_NOT_FOUND) {
                throw new IOException("publisher revoke failed: HTTP " + status);
            }
        } finally { if (connection != null) connection.disconnect(); }
    }

    private static Uri buildHandoffUri(String playlist, String name, String owner, String pub) throws Exception {
        long issuedAt = System.currentTimeMillis();
        long expiresAt = issuedAt + 60_000L;
        String nonce = UUID.randomUUID().toString().replace("-", "");
        String packageName = "com.fadcam";
        String canonical = "1|" + nonce + "|" + issuedAt + "|" + expiresAt + "|" + packageName + "|" + playlist + "|" + name + "|" + owner;
        String signature = Base64.encodeToString(sign(canonical.getBytes(StandardCharsets.UTF_8)), Base64.URL_SAFE | Base64.NO_WRAP | Base64.NO_PADDING);
        return new Uri.Builder().scheme("fadcam").authority("stream").appendQueryParameter("v", "1")
                .appendQueryParameter("nonce", nonce).appendQueryParameter("iat", Long.toString(issuedAt))
                .appendQueryParameter("exp", Long.toString(expiresAt)).appendQueryParameter("package", packageName)
                .appendQueryParameter("name", name).appendQueryParameter("owner", owner).appendQueryParameter("url", playlist)
    ...
    private static byte[] sign(byte[] data) throws Exception {
        ...
        Signature signature = Signature.getInstance("SHA256withECDSA"); signature.initSign(privateKey); signature.update(data); return signature.sign();
    }

    private static String fingerprint(byte[] der) throws Exception {
        byte[] digest = MessageDigest.getInstance("SHA-256").digest(der); StringBuilder out = new StringBuilder(digest.length * 2);
    ...
    private static String postRaw(String endpoint, String body) throws Exception {
        HttpURLConnection connection = null;
        try {
            connection = (HttpURLConnection) new URL(endpoint).openConnection(); connection.setRequestMethod("POST"); connection.setDoOutput(true);
            connection.setConnectTimeout(CONNECT_TIMEOUT_MS); connection.setReadTimeout(READ_TIMEOUT_MS); connection.setRequestProperty("Content-Type", "application/json");
            byte[] bytes = body.getBytes(StandardCharsets.UTF_8); connection.setFixedLengthStreamingMode(bytes.length);
            try (OutputStream out = connection.getOutputStream()) { out.write(bytes); }
            int status = connection.getResponseCode(); String response = readBounded(status >= 400 ? connection.getErrorStream() : connection.getInputStream());
            if (status < 200 || status >= 300) throw new IOException("publisher request rejected: HTTP " + status + " " + response); return response;
        } finally { if (connection != null) connection.disconnect(); }
    }

    private static String readBounded(InputStream input) throws IOException {
        if (input == null) return ""; ByteArrayOutputStream out = new ByteArrayOutputStream(); byte[] buffer = new byte[4096]; int total = 0; int read;
    ...
    private static String jsonString(String json, String key) {
        ...
        while (start < json.length() && Character.isWhitespace(json.charAt(start))) start++; if (start >= json.length() || json.charAt(start) != '\"') return null; start++;
