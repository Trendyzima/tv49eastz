...
public final class FadCamTvPublisher {
    ...
    public static void start(@NonNull Context context, @NonNull String channelId,
        ...
        EXECUTOR.execute(() -> {
            try {
                ...
                String requestCanonical = publisherCanonical(1, nonce, iat, deviceId, channel, stream);
                ...
                request.put("channel_id", channel);
                request.put("stream_id", stream);
                ...
                if (!playlist.startsWith("https://")) throw new SecurityException("gateway playlist is not HTTPS");

                long nowMs = System.currentTimeMillis();
                long expMs = Math.min(nowMs + MAX_HANDOFF_LIFETIME_MS, nowMs + expires * 1000L);
                long iatMs = iat * 1000L;
                String handoffCanonical = FadCamHandoffVerifier.canonical(
                        1, nonce, iatMs, expMs, PACKAGE, playlist, NAME, OWNER);
                String handoffSignature = sign(handoffCanonical);

                Uri handoff = new Uri.Builder()
                        .scheme("fadcam").authority("stream")
                        .appendQueryParameter("v", "1")
                        .appendQueryParameter("nonce", nonce)
                        .appendQueryParameter("iat", Long.toString(iatMs))
                        .appendQueryParameter("exp", Long.toString(expMs))
    ...
    /** Revokes the gateway session; it does not stop the local Server Room. */
    ...
    private static String publisherCanonical(int version, String nonce, long issuedAt,
                                             String deviceId, String channelId, String streamId) {
        return version + "|" + nonce + "|" + issuedAt + "|" + deviceId + "|" + channelId + "|" + streamId;
    }
    ...
    private static void delete(String endpoint) throws Exception {
        ...
        int status = connection.getResponseCode();
        connection.disconnect();
        if (status != 204 && status != 200 && status != 404) throw new IllegalStateException("publisher revoke failed: HTTP " + status);
    }
